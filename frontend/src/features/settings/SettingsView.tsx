...
export const SettingsView = ({ onClose, onLogout, initialTab }: any) => {
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            addToast(t('settings.profile.updateSuccess'), 'success');
        } catch (e) {
            console.error(e);
            addToast(t('settings.profile.updateError'), 'error');
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (updateRes.ok) {
                await refreshProfile();
                addToast(t('settings.profile.avatarSuccess'), 'success');
            }
        } catch (e) {
            addToast(t('settings.profile.avatarError'), 'error');
        }
    };
    ...
    const handleGenerateKey = async () => {
        try {
            ...
            addToast(t('settings.api.createSuccess'), 'success');
        } catch (e) {
            console.error(e);
            addToast(t('settings.api.createError'), 'error');
        }
    };
    ...
    const handleRevokeKey = async (keyToRevoke: string) => {
        try {
            ...
            addToast(t('settings.api.revokeSuccess'), 'success');
        } catch (e) {
            console.error(e);
            addToast(t('settings.api.revokeError'), 'error');
        }
    };
    ...
    const NOTIFICATION_CATEGORIES = [
        {
            title: t('settings.notifications.cat.collaboration'),
            description: t('settings.notifications.cat.collaborationDesc'),
            options: [
                { key: 'direct_messages', title: t('settings.notifications.opt.dm'), desc: t('settings.notifications.opt.dmDesc') },
                { key: 'mentions', title: t('settings.notifications.opt.mentions'), desc: t('settings.notifications.opt.mentionsDesc') },
                { key: 'thread_replies', title: t('settings.notifications.opt.threads'), desc: t('settings.notifications.opt.threadsDesc') }
            ]
        },
        {
            title: t('settings.notifications.cat.intelligence'),
            description: t('settings.notifications.cat.intelligenceDesc'),
            options: [
                { key: 'analysis_complete', title: t('settings.notifications.opt.analysis'), desc: t('settings.notifications.opt.analysisDesc') },
                { key: 'automl_training', title: t('settings.notifications.opt.automl'), desc: t('settings.notifications.opt.automlDesc') },
                { key: 'anomaly_detected', title: t('settings.notifications.opt.anomaly'), desc: t('settings.notifications.opt.anomalyDesc') },
                { key: 'scheduled_reports', title: t('settings.notifications.opt.reports'), desc: t('settings.notifications.opt.reportsDesc') }
            ]
        },
        {
            title: t('settings.notifications.cat.infrastructure'),
            description: t('settings.notifications.cat.infrastructureDesc'),
            options: [
                { key: 'sync_failure', title: t('settings.notifications.opt.sync'), desc: t('settings.notifications.opt.syncDesc') },
                { key: 'schema_change', title: t('settings.notifications.opt.schema'), desc: t('settings.notifications.opt.schemaDesc') },
                { key: 'pipeline_success', title: t('settings.notifications.opt.pipeline'), desc: t('settings.notifications.opt.pipelineDesc') }
            ]
        },
        {
            title: t('settings.notifications.cat.security'),
            description: t('settings.notifications.cat.securityDesc'),
            options: [
                { key: 'new_logins', title: t('settings.notifications.opt.logins'), desc: t('settings.notifications.opt.loginsDesc') },
                { key: 'role_changes', title: t('settings.notifications.opt.roles'), desc: t('settings.notifications.opt.rolesDesc') },
                { key: 'security', title: t('settings.notifications.opt.security'), desc: t('settings.notifications.opt.securityDesc') },
                { key: 'updates', title: t('settings.notifications.opt.updates'), desc: t('settings.notifications.opt.updatesDesc') }
            ]
        }
    ];
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
                <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '40px 32px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '32px' }} className="fade-in">
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
                                        {avatarUrl ? (
                                            <img src={avatarUrl} alt="Profile" style={{ width: '96px', height: '96px', borderRadius: '50%', objectFit: 'cover', boxShadow: '0 8px 24px rgba(99,102,241,0.3)' }} />
                                        ) : (
                                            <div style={{ width: '96px', height: '96px', borderRadius: '50%', background: 'linear-gradient(135deg, #6366f1, #ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', fontWeight: 'bold', color: 'white' }}>
                                                {user?.email?.[0]?.toUpperCase() || 'U'}
                                            </div>
                                        )}
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            <button className="btn btn-secondary btn-sm" onClick={() => fileInputRef.current?.click()}>{t('settings.profile.upload')}</button>
                                            <input type="file" ref={fileInputRef} style={{ display: 'none' }} accept="image/*" onChange={handleAvatarUpload} />
                                            <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{t('settings.profile.uploadHint')}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                                        ...
                                        <div style={{ gridColumn: 'span 2', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            ...
                                            <textarea className="input" style={{ height: '120px', resize: 'none' }} placeholder={t('settings.profile.bioPlaceholder')} value={bio} onChange={e => setBio(e.target.value)} maxLength={240} />
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    {[
                                        ...
                                        >
                                            ...
                                            <div style={{ flex: 1, textAlign: 'left', display: 'flex', flexDirection: 'column' }}>
                                                ...
                                                <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>{thm.desc}</span>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div style={{ maxWidth: '900px' }} className="fade-in">
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                                    <div>
                                        ...
                                        <p style={{ fontSize: '14px', color: 'var(--text-secondary)' }}>{t('settings.api.manage')}</p>
                                    </div>
                                    <button className="btn btn-primary" onClick={handleGenerateKey}>{t('settings.api.create')}</button>
                                </div>

                                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                                    {apiKeys.length === 0 ? (
                                        <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)', background: 'var(--bg-surface)', borderRadius: '12px' }}>{t('settings.api.empty')}</div>
                                    ) : (
                                        apiKeys.map((keyObj, i) => (
                                            <div key={i} className="card" style={{ padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                                                    <Icons.Key />
                                                    <div>
                                                        <code style={{ fontSize: '14px' }}>{keyObj.key.substring(0, 8)}...{keyObj.key.substring(keyObj.key.length - 8)}</code>
                                                        <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{t('settings.api.created')} {new Date(keyObj.createdAt).toLocaleDateString()}</p>
                                                    </div>
                                                </div>
                                                <div style={{ display: 'flex', gap: '8px' }}>
                                                    <button className="btn btn-secondary btn-sm" onClick={() => { navigator.clipboard.writeText(keyObj.key); addToast(t('settings.api.copySuccess'), 'success'); }}>{t('settings.api.copy')}</button>
                    ...
                    {activeTab === 'notifications' && (
                        <div style={{ maxWidth: '900px' }} className="fade-in">
                            <h3 className="text-h3" style={{ marginBottom: '8px' }}>{t('settings.notifications.title')}</h3>
                            <p style={{ fontSize: '14px', color: 'var(--text-secondary)', marginBottom: '32px' }}>{t('settings.notifications.desc')}</p>

                            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                                {NOTIFICATION_CATEGORIES.map((category, catIdx) => (
                                    <div key={catIdx}>
                                        <h4 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '4px' }}>{category.title}</h4>
                                        <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginBottom: '16px' }}>{category.description}</p>
                                        <div className="card" style={{ padding: 0 }}>
                                            {category.options.map((pref, i) => (
                                                <div key={pref.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: i < category.options.length - 1 ? '1px solid var(--border-default)' : 'none' }}>
                                                    <div>
                                                        <div style={{ fontWeight: 600, fontSize: '14px' }}>{pref.title}</div>
                                                        <div style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>{pref.desc}</div>
                                                    </div>
                                                    <label className="switch">
                                                        <input type="checkbox" checked={!!notificationPrefs[pref.key]} onChange={() => handleNotificationToggle(pref.key)} />
