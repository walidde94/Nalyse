...
export const WebhookSystemView = ({ token }: { token?: string }) => {
    ...
    const CopyBtn = ({ text, id }: { text: string; id: string }) => (
        <button onClick={() => copyText(text, id)} style={{ background: 'none', border: 'none', color: copiedId === id ? '#10b981' : 'rgba(255,255,255,0.4)', cursor: 'pointer', padding: '4px', borderRadius: '4px', display: 'inline-flex' }}>
            {copiedId === id ? <Check size={13} /> : <Copy size={13} />}
        </button>
    );

    const StatCard = ({ label, value, sub, color, icon: Icon }: any) => (
        <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '20px', flex: 1, minWidth: '160px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ position: 'absolute', top: '-10px', right: '-10px', opacity: 0.06 }}><Icon size={60} /></div>
            <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.45)', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '8px' }}>{label}</div>
            <div style={{ fontSize: '28px', fontWeight: 900, color: color || '#fff', fontFamily: 'var(--font-mono)', lineHeight: 1 }}>{value}</div>
            {sub && <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginTop: '6px', fontWeight: 600 }}>{sub}</div>}
        </div>
    );
        ...
        <div style={{ padding: '32px', maxWidth: '1400px', margin: '0 auto', fontFamily: 'var(--font-main)', position: 'relative', minHeight: '100%' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '28px', position: 'relative', zIndex: 1 }}>
                <div>
                    ...
                    <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.45)', marginTop: '6px', fontWeight: 500, marginLeft: '54px' }}>
                        Production-grade event delivery, API key management, and real-time health monitoring.
                    </p>
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                    <button onClick={fetchAll} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: '10px 16px', borderRadius: '10px', color: 'rgba(255,255,255,0.6)', fontSize: '12px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                ...
                {tabs.map(t => {
                    ...
                    return (
                        <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
                            ...
                            {t.icon} {t.label}
                            {t.count !== undefined && <span style={{ background: active ? 'rgba(245,158,11,0.2)' : 'rgba(255,255,255,0.05)', padding: '2px 8px', borderRadius: '20px', fontSize: '10px', fontWeight: 800, color: active ? '#f59e0b' : 'rgba(255,255,255,0.3)' }}>{t.count}</span>}
                        </button>
                    );
                })}
            </div>
            ...
            <AnimatePresence>
                {showCreate && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                        style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', padding: '28px', marginBottom: '24px', position: 'relative', zIndex: 1 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                            <div>
                                <label style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', fontWeight: 700, display: 'block', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Payload URL</label>
                                <input value={newUrl} onChange={e => setNewUrl(e.target.value)} placeholder="https://api.yourapp.com/webhooks/nalyse" style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', padding: '12px 16px', borderRadius: '8px', color: '#fff', fontSize: '13px', outline: 'none', boxSizing: 'border-box', fontFamily: 'var(--font-mono)' }} />
                            </div>
                            <div>
                                <label style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', fontWeight: 700, display: 'block', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Description (optional)</label>
                                <input value={newDesc} onChange={e => setNewDesc(e.target.value)} placeholder="e.g. Slack alerts, ERP sync, CI trigger..." style={{ width: '100%', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', padding: '12px 16px', borderRadius: '8px', color: '#fff', fontSize: '13px', outline: 'none', boxSizing: 'border-box' }} />
                            </div>
                            <div>
                                <label style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', fontWeight: 700, display: 'block', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Subscribe to Events</label>
                                {Object.entries(eventsByCategory).map(([cat, evts]) => (
                                    <div key={cat} style={{ marginBottom: '10px' }}>
                                        <div style={{ fontSize: '10px', fontWeight: 800, color: 'rgba(255,255,255,0.3)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{cat}</div>
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                            {evts.map(ev => {
                                                ...
                                                return <button key={ev.id} onClick={() => setSelectedEvents(sel ? selectedEvents.filter(e => e !== ev.id) : [...selectedEvents, ev.id])}
                                                    ...
                                                    style={{ padding: '6px 12px', borderRadius: '6px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-mono)', border: `1px solid ${sel ? '#f59e0b' : 'rgba(255,255,255,0.1)'}`, background: sel ? 'rgba(245,158,11,0.15)' : 'transparent', color: sel ? '#f59e0b' : 'rgba(255,255,255,0.5)', transition: 'all 0.2s' }}>
                                                    {ev.id}
                                                </button>;
                                            })}
                                        </div>
            ...
            <div style={{ position: 'relative', zIndex: 1 }}>
                {loading ? (
                    <div style={{ height: '300px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px' }}>
                        ...
                        <span style={{ fontSize: '12px', fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Loading Engine...</span>
                    </div>
                ) : (
                    <AnimatePresence mode="wait">
                        <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.25 }}>
                            ...
                            {activeTab === 'overview' && stats && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                    ...
                                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '24px' }}>
                                        <h3 style={{ fontSize: '14px', fontWeight: 800, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}><Activity size={16} color="#3b82f6" /> Recent Activity</h3>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                            {logs.slice(0, 6).map(log => (
                                                <div key={log.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.03)' }}>
                                                    ...
                                                    <code style={{ fontSize: '12px', color: 'rgba(255,255,255,0.7)', fontFamily: 'var(--font-mono)', flex: 1 }}>{log.event}</code>
                                                    <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', fontFamily: 'var(--font-mono)' }}>{log.duration}ms</span>
                                                    <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)' }}>{timeAgo(log.timestamp)}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                        ...
                                        <motion.div key={wh.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                                                <div>
                                                    ...
                                                    {wh.description && <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginBottom: '8px', marginLeft: '22px' }}>{wh.description}</div>}
                                            ...
                                            <div style={{ display: 'flex', gap: '24px', paddingTop: '14px', borderTop: '1px solid rgba(255,255,255,0.05)', flexWrap: 'wrap' }}>
                                                {[
                                                    ...
                                                    <div key={i}>
                                                        <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{s.label}</div>
                                                        <div style={{ fontSize: '13px', fontWeight: 700, color: s.color || 'rgba(255,255,255,0.7)', fontFamily: s.mono ? 'var(--font-mono)' : 'inherit', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                                            {s.icon}{s.value}
                                                        </div>
                                                    </div>
                                                ))}
                                                <div style={{ marginLeft: 'auto' }}>
                                                    <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)', fontWeight: 700, textTransform: 'uppercase' }}>Secret</div>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
                                                        <code style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', fontFamily: 'var(--font-mono)' }}>{visibleSecrets.has(wh.id) ? wh.secret : wh.secret.substring(0, 12) + '•••'}</code>
                                                        <button onClick={() => toggleSecret(wh.id)} style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.3)', cursor: 'pointer', padding: '2px' }}>{visibleSecrets.has(wh.id) ? <EyeOff size={12} /> : <Eye size={12} />}</button>
                            ...
                            {activeTab === 'apikeys' && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '24px' }}>
                                        <h3 style={{ fontSize: '14px', fontWeight: 800, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}><Key size={16} color="#f59e0b" /> Generate New Key</h3>
                                        <div style={{ display: 'flex', gap: '10px' }}>
                                            <input value={newKeyName} onChange={e => setNewKeyName(e.target.value)} placeholder="Key name (e.g. Production Backend)" style={{ flex: 1, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', padding: '12px 16px', borderRadius: '8px', color: '#fff', fontSize: '13px', outline: 'none' }} />
                                        ...
                                        <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)', marginTop: '10px' }}><Lock size={10} style={{ display: 'inline', verticalAlign: '-1px' }} /> Keys are shown once at creation. Store them securely — never expose in client-side code.</p>
                                    </div>
                                    {apiKeys.map((k: any, i: number) => (
                                        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '16px', padding: '18px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                            <Shield size={16} color="#f59e0b" />
                                            <div style={{ flex: 1 }}>
                                                <div style={{ fontSize: '13px', fontWeight: 700, color: '#fff' }}>{k.name || 'Unnamed Key'}</div>
                                                <code style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'rgba(255,255,255,0.5)' }}>{k.key?.substring(0, 20)}•••</code>
                                            </div>
                                            <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)' }}>{k.createdAt ? new Date(k.createdAt).toLocaleDateString() : ''}</span>
                                    ...
                                    {logs.map((log, i) => (
                                        <motion.div key={log.id} initial={{ opacity: 0, x: -5 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}>
                                            <div onClick={() => setExpandedLog(expandedLog === log.id ? null : log.id)}
                                                style={{ display: 'grid', gridTemplateColumns: '70px 1fr 90px 80px 80px 30px', alignItems: 'center', padding: '12px 16px', background: expandedLog === log.id ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.02)', border: `1px solid ${expandedLog === log.id ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.03)'}`, borderRadius: expandedLog === log.id ? '10px 10px 0 0' : '10px', gap: '12px', cursor: 'pointer', transition: 'all 0.15s' }}>
                                                ...
                                                <code style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: 'rgba(255,255,255,0.7)' }}>{log.event}</code>
                                                <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>{timeAgo(log.timestamp)}</span>
                                                <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: log.duration > 2000 ? '#ef4444' : 'rgba(255,255,255,0.5)' }}>{log.duration}ms</span>
                                                <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.3)' }}>attempt {log.attempt}</span>
                                                <ChevronDown size={14} style={{ color: 'rgba(255,255,255,0.3)', transform: expandedLog === log.id ? 'rotate(180deg)' : 'rotate(0)', transition: '0.2s' }} />
                                            </div>
                                            {expandedLog === log.id && (
                                                <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.08)', borderTop: 'none', borderRadius: '0 0 10px 10px', padding: '16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                                                    <div>
                                                        <div style={{ fontSize: '10px', fontWeight: 800, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: '8px' }}>Request Body</div>
                                                        <pre style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: 'rgba(255,255,255,0.6)', background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px', margin: 0, whiteSpace: 'pre-wrap', maxHeight: '200px', overflow: 'auto' }}>{log.requestBody || 'N/A'}</pre>
                                                    </div>
                                                    <div>
                                                        <div style={{ fontSize: '10px', fontWeight: 800, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', marginBottom: '8px' }}>Response Body</div>
                                                        <pre style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: log.status < 300 ? 'rgba(16,185,129,0.8)' : 'rgba(239,68,68,0.8)', background: 'rgba(0,0,0,0.2)', padding: '12px', borderRadius: '8px', margin: 0, whiteSpace: 'pre-wrap', maxHeight: '200px', overflow: 'auto' }}>{log.responseBody || 'N/A'}</pre>
                                                    </div>
                                                </div>
                                            )}
                                        </motion.div>
  ...
  "timestamp": "2026-04-05T01:00:00Z"
                                    ...
                                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '24px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                                            <h3 style={{ fontSize: '14px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '8px', margin: 0 }}><Shield size={16} color="#10b981" /> Signature Verification — Real Example</h3>
                                            <div style={{ display: 'flex', gap: '4px' }}>
                                                {(['node', 'python', 'go'] as const).map(lang => (
                                                    <button key={lang} onClick={() => setDocLang(lang)} style={{ padding: '6px 14px', borderRadius: '6px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', border: '1px solid', borderColor: docLang === lang ? '#10b981' : 'rgba(255,255,255,0.1)', background: docLang === lang ? 'rgba(16,185,129,0.15)' : 'transparent', color: docLang === lang ? '#10b981' : 'rgba(255,255,255,0.4)', textTransform: 'capitalize' }}>{lang === 'node' ? 'Node.js' : lang === 'python' ? 'Python' : 'Go'}</button>
                                                ))}
                                            </div>
                                        </div>
                                        <div style={{ position: 'relative' }}>
                                            <pre style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', background: 'rgba(0,0,0,0.3)', padding: '20px', borderRadius: '10px', margin: 0, color: 'rgba(255,255,255,0.75)', lineHeight: 1.7, whiteSpace: 'pre-wrap', overflow: 'auto', maxHeight: '500px' }}>{verifyExamples[docLang]}</pre>
                                            <button onClick={() => copyText(verifyExamples[docLang], `verify-${docLang}`)} style={{ position: 'absolute', top: '12px', right: '12px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', padding: '6px 12px', borderRadius: '6px', fontSize: '10px', fontWeight: 700, cursor: 'pointer', color: copiedId === `verify-${docLang}` ? '#10b981' : 'rgba(255,255,255,0.5)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                    ...
                                    {/* Available Events */}
                                    <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '24px' }}>
                                        <h3 style={{ fontSize: '14px', fontWeight: 800, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}><Hash size={16} color="#f59e0b" /> Available Event Types</h3>
                                        {Object.entries(eventsByCategory).map(([cat, evts]) => (
                                            <div key={cat} style={{ marginBottom: '16px' }}>
                                                <div style={{ fontSize: '10px', fontWeight: 800, color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '8px' }}>{cat}</div>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                    {evts.map(ev => (
                                                        <div key={ev.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 14px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px' }}>
                                                            <code style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: '#f59e0b', fontWeight: 700, minWidth: '180px' }}>{ev.id}</code>
                                                            <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>{ev.description}</span>
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
