...
export const SettingsTab = () => {
    ...
    const L: React.CSSProperties = { fontSize: 10, fontWeight: 800, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 };
    const Section = ({ icon: Icon, title, children }: { icon: any; title: string; children: React.ReactNode }) => (
        <div className="glass-panel" style={{ padding: 28, borderRadius: 20, border: '1px solid var(--border-default)' }}>
            <h3 style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-primary)', marginBottom: 22, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                    ...
                    <div><div style={L}>Default Format</div>
                        <div style={{ display: 'flex', gap: 8 }}>
                            {['html','pdf','csv','json'].map(f => <button key={f} onClick={() => u('defaultFormat', f)} style={{ flex: 1, padding: 10, borderRadius: 10, fontSize: 10, fontWeight: 800, textTransform: 'uppercase', background: settings.defaultFormat === f ? '#6366f1' : 'var(--bg-surface-hover)', border: 'none', color: '#fff', cursor: 'pointer' }}>{f}</button>)}
                        </div>
                    </div>
                    <div><div style={L}>Default Priority</div>
                        <div style={{ display: 'flex', gap: 8 }}>
                            {['low','normal','high','critical'].map(p => <button key={p} onClick={() => u('defaultPriority', p)} style={{ flex: 1, padding: 10, borderRadius: 10, fontSize: 10, fontWeight: 800, textTransform: 'uppercase', background: settings.defaultPriority === p ? '#6366f1' : 'var(--bg-surface-hover)', border: 'none', color: '#fff', cursor: 'pointer' }}>{p}</button>)}
                        </div>
                    </div>
                </div>
            </Section>
            ...
            <Section icon={Bell} title="Notifications">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                    {[
                        ...
                        <label key={n.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderRadius: 12, background: 'var(--bg-surface)', border: '1px solid var(--border-default)', cursor: 'pointer' }}>
                            <span style={{ fontSize: 13, color: 'var(--text-secondary)', fontWeight: 500 }}>{n.label}</span>
                            <div onClick={() => u(n.key, !settings[n.key])} style={{ width: 40, height: 22, borderRadius: 11, background: settings[n.key] ? '#6366f1' : 'var(--bg-elevated)', transition: 'background 0.2s', position: 'relative', cursor: 'pointer' }}>
