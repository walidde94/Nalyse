...
export type SettingsNavTabV1 = 'profile' | 'appearance' | 'security' | 'api' | 'notifications' | 'subscription';
...
interface HeaderProps {
    ...
    onGlassToggle?: () => void;
    onMenuToggle?: () => void;
    onNavigate?: (path: string, options?: { settingsTab?: SettingsNavTabV1 }) => void;
}

export const Header: React.FC<HeaderProps> = ({ theme, glassMode = false, onThemeToggle, onGlassToggle, onMenuToggle, onNavigate }) => {
    const { user } = useAuth();
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <div style={{ position: 'relative' }}>
                            ...
                            {/* Notifications Dropdown - Premium Intelligence Feed */}
                            {showNotifications && (
                                <NotificationCenter
                                    isDark={isDark}
                        ...
                        {onGlassToggle && (
                            <button
                                ...
                                className="nexus-icon-btn"
                                title={`Glass effect: ${glassMode ? 'On' : 'Off'}`}
                                style={{
                                    position: 'relative',
                                    ...(glassMode ? {
                                        background: isDark ? 'rgba(99, 102, 241, 0.12)' : 'rgba(37, 99, 235, 0.08)',
                                        borderColor: isDark ? 'rgba(99, 102, 241, 0.3)' : 'rgba(37, 99, 235, 0.2)',
                                        boxShadow: `0 0 12px ${isDark ? 'rgba(99, 102, 241, 0.15)' : 'rgba(37, 99, 235, 0.1)'}`,
                                    } : {})
                                }}
                            >
                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ transition: 'opacity 0.3s', opacity: glassMode ? 1 : 0.6 }}>
                                    <rect x="2" y="4" width="20" height="16" rx="3" ry="3" strokeDasharray={glassMode ? '0' : '4 2'} />
                                ...
                                {glassMode && (
                                    <div style={{
                                        ...
                                        boxShadow: `0 0 6px ${isDark ? '#818cf8' : '#3b82f6'}`,
                                    }} />
                                )}
                            </button>
                        )}
                            ...
                            <div style={{
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                    {[
                                        ...
                                        { title: 'Core Settings', sub: 'System Parameters', icon: Settings, color: '#3b82f6', bg: 'rgba(59, 130, 246, 0.1)', settingsTab: 'appearance' as const }
                                    ].map((item, i) => (
                                        <button
                                            key={i}
