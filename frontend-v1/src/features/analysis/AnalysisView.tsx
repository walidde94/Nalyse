...
export const AnalysisView = ({ analysis, onClose, onShare, onUpgradeRequested }: AnalysisViewProps) => {
    ...
    const [nlqSql, setNlqSql] = useState<string | null>(null);
    ...
    // KQL Parser (Advanced Pro Version) & NLQ Fallback
    const parseQuery = async (query: string) => {
        ...
        if (hasKqlMatch || newSearchTerms.length === 0) {
            // Classical filtering
            setNlqSql(null);
            setGlobalFilters({ ...newGlobalFilters, _global_search: newSearchTerms });
        } else {
            // NLQ Translation Flow
            // If it's pure natural language, send to AI service
            ...
            try {
                ...
                const schema = {
                    dimensions,
                    measures,
                    colTypes: analysis.summary?.columnTypes || {}
                };

                const res = await fetch(`${API_URL_BASE}/api/ai/nlq`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                    body: JSON.stringify({ query, schema })
                });

                if (!res.ok) throw new Error('NLQ Failed');
                const data = await res.json();

                if (data.sql) {
                    setNlqSql(data.sql);
                    setGlobalFilters({ _global_search: [query] }); // Just for display
                }
            } catch (err) {
                console.error("NLQ Translation Error:", err);
                addToast("Failed to interpret standard query. Falling back to search.", "warning");
                setGlobalFilters({ _global_search: [query] });
                setNlqSql(null);
            } finally {
    ...
    useEffect(() => {
        ...
        // Apply global text search (if any) and handle display-only if nlqSql is active
        if (globalFilters._global_search && !nlqSql) {
        ...
        // Apply NLQ SQL if it exists
        if (nlqSql) {
            try {
                // Remove LIMIT/ORDER BY to prevent alasql from throwing if we only want filtered subset
                // Only if the result count differs significantly. AlaSQL can apply WHERE seamlessly.
                const res = alasql(nlqSql, [filtered]) as any[];
                if (res && Array.isArray(res)) {
                    filtered = res;
                }
            } catch (e) {
                console.error("Failed to apply NLQ SQL filters", e);
            }
        }

        setFilteredData(filtered);
    }, [globalFilters, dateRange, activeDrillDown, localData, nlqSql]);
        ...
        <div className="flex h-screen" style={{ background: 'var(--bg-app)', color: 'var(--text-primary)' }}>
            ...
            <div className="flex-col" style={{ flex: 1, overflow: 'hidden', minWidth: 0 }}>
                <div className="flex-1" style={{ overflowY: 'auto' }} id="analysis-content">
                    ...
                    <div className={activeTab === 'presentation' ? 'hidden' : 'relative'} style={{ position: 'sticky', top: '72px', zIndex: 30, background: 'var(--bg-app)', borderBottom: '1px solid var(--border-subtle)' }}>
                        <ElasticSearch
                            onSearch={parseQuery}
                        ...
                        <AnimatePresence>
                            {isAiThinking && (
                                <motion.div
                                    ...
                                    className="absolute right-3 top-2 flex items-center gap-2 pointer-events-none"
                                >
                                    <span className="text-xs font-bold text-primary animate-pulse tracking-widest uppercase">NLQ Synth</span>
                                    <div className="relative flex items-center justify-center w-8 h-8">
                                        <div className="absolute inset-0 bg-primary/20 rounded-full animate-ping" />
                                        <div className="absolute inset-1 bg-primary/40 rounded-full animate-pulse" />
                                        <div className="absolute inset-2 bg-primary rounded-full shadow-[0_0_15px_var(--primary)]" />
                                        <BrainCircuit size={12} className="text-white relative z-10 animate-pulse" />
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
