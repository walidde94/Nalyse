...
export const AnalysisMapView = ({ data }: AnalysisMapViewProps) => {
        ...
        return (
            <div className="flex items-center justify-center p-20 card" style={{ minHeight: '400px', background: 'var(--bg-surface)' }}>
                <div className="text-center flex-col gap-4">
                    ...
                    <h3 className="text-h3">No Data Available</h3>
                    <p className="text-sec">Upload a dataset with rows to visualize on the map.</p>
                </div>
            </div>
        );
    }
    ...
    if (!geoColumns) {
        return (
            <div className="flex items-center justify-center p-20 card" style={{ minHeight: '400px', background: 'var(--bg-surface)' }}>
                <div className="text-center flex-col gap-4" style={{ maxWidth: '400px' }}>
                    <div style={{ color: 'var(--primary)' }}><MapPin size={48} /></div>
                    <h3 className="text-h3">No Geo Data Detected</h3>
                    <p className="text-sec">This dataset doesn't contain recognized latitude/longitude columns.</p>
                    <div style={{
                        marginTop: '8px', padding: '12px 16px', borderRadius: '10px',
                        background: 'rgba(99,102,241,0.05)', border: '1px solid rgba(99,102,241,0.15)',
                        fontSize: '11px', color: 'var(--text-secondary)', textAlign: 'left',
                    }}>
                        <div style={{ fontWeight: 700, marginBottom: '6px', color: 'var(--text-primary)' }}>Expected column names:</div>
                        <code style={{ fontSize: '10px', color: '#818cf8' }}>lat, latitude, lng, long, longitude</code>
                        <div style={{ marginTop: '8px', fontWeight: 700, color: 'var(--text-primary)' }}>Example:</div>
                        <code style={{ fontSize: '10px', color: '#818cf8' }}>city, latitude, longitude, sales</code>
                    </div>
                </div>
            </div>
        );
