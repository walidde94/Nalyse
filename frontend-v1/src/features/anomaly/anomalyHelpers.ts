...
export function runAnomalyDetection(
    ...
    numericCols.forEach((col, colIdx) => {
        ...
        const timestamps = data.map(r => new Date(r[tsCol]));
        const hasValidDates = timestamps.every(d => !isNaN(d.getTime()));
        const dayAverages = new Array(7).fill(0).map(() => ({ sum: 0, count: 0 }));

        if (hasValidDates) {
            timestamps.forEach((d, i) => {
                const day = d.getDay();
                dayAverages[day].sum += values[i];
                dayAverages[day].count++;
            });
        }

        const globalMean = mean(values) || 1;
        const seasonalFactors = new Array(7).fill(1);

        if (hasValidDates) {
            dayAverages.forEach((d, i) => {
                seasonalFactors[i] = d.count > 0 ? (d.sum / d.count) / globalMean : 1;
            });
        }

        // Seasonally adjust the inputs for detection
        const adjustedValues = values.map((v, i) => {
            if (!hasValidDates) return v;
            const factor = seasonalFactors[timestamps[i].getDay()] || 1;
        ...
        // Compute moving average on the *adjusted* values so we track the baseline properly
        const rMeanAdjusted = rollingMean(adjustedValues, config.windowSize);
        const rStd = rollingStd(values, config.windowSize);

        // Combined detection using SEASONALLY ADJUSTED values
        let flags: boolean[];
        if (config.method === 'zscore') {
            flags = detectZScore(adjustedValues, threshold);
        } else if (config.method === 'iqr') {
            flags = detectIQR(adjustedValues, iqrMultiplier);
        } else {
            // Auto: combine methods for robustness
            const zFlags = detectZScore(adjustedValues, threshold);
            const mzFlags = detectModifiedZScore(adjustedValues, threshold);
            const iqrFlags = detectIQR(adjustedValues, iqrMultiplier);
            flags = adjustedValues.map((_, i) => {
        ...
        flags.forEach((isAnomaly, i) => {
            if (isAnomaly) {
                ...
                const val = values[i];
                // Multiply back the seasonal factor to get the actual expected absolute value
                const factor = hasValidDates ? seasonalFactors[timestamps[i].getDay()] : 1;
                const expectedNormalized = rMeanAdjusted[i] || mean(adjustedValues);
                const expected = expectedNormalized * factor;
